...
gap_base <- rand_long %>%
  ...
  summarise(
    ...
    first_gap_year = {
      cand <- year[is_gap & year > 2010  & year <= 2016]
      if (length(cand) == 0) NA_real_ else min(cand)
    },
...
drop_counts <- analysis_ready %>%
  group_by(algorithm) %>%
  summarise(n_drop = sum(drop_zero_time, na.rm = TRUE), .groups = "drop")
drop_counts
...
# join apoe dataset ----
apoe_filter <- apoe_data %>% select(hhidpn, apoe)
...
analysis_ready <- analysis_ready %>%
  mutate(
    elig_table1 = case_when(
      ...
      is.na(dem2010)                                ~ 0L,
      algorithm == "wu"     & race_ethnicity == "3" ~ 0L,
      algorithm == "expert" & race_ethnicity == "4" ~ 0L,
      TRUE                                          ~ 1L
    ),
    elig_analysis = if_else(elig_table1 == 1L & dem2010 == 0L, 1L, 0L)
  )
...
analysis_ready_age70 <- analysis_ready %>%
  mutate(
    ...
    elig_analysis_70 = if_else(elig_table1_70 == 1L & dem2010 == 0L, 1L, 0L)
  )

# Datasets (sensitivity: 2016 censoring) ----
## age >= 50
dt_tab1_age50_sens     <- analysis_ready %>% filter(elig_table1 == 1L)
dt_analysis_age50_sens <- analysis_ready %>% filter(elig_analysis == 1L)

## age >= 70
dt_tab1_age70_sens     <- analysis_ready_age70 %>% filter(elig_table1_70 == 1L)
dt_analysis_age70_sens <- analysis_ready_age70 %>% filter(elig_analysis_70 == 1L)
...
dt_tab1_age50_sens <- dt_tab1_age50_sens %>%
  filter(complete.cases(across(all_of(model_vars))))

dt_analysis_age50_sens <- dt_analysis_age50_sens %>%
  filter(complete.cases(across(all_of(model_vars)))) %>%
  select(-starts_with("dem_3lw"), -starts_with("dem_3hudomiet"))

dt_tab1_age70_sens <- dt_tab1_age70_sens %>%
  filter(complete.cases(across(all_of(model_vars))))

dt_analysis_age70_sens <- dt_analysis_age70_sens %>%
  filter(complete.cases(across(all_of(model_vars)))) %>%
  select(-starts_with("dem_3lw"), -starts_with("dem_3hudomiet"))
...
# saveRDS(dt_tab1_age50_sens,     file = file.path(save_path, "dt_tab1_age50_sens2016.rds"))
# saveRDS(dt_analysis_age50_sens, file = file.path(save_path, "dt_analysis_age50_sens2016.rds"))
# saveRDS(dt_tab1_age70_sens,     file = file.path(save_path, "dt_tab1_age70_sens2016.rds"))
# saveRDS(dt_analysis_age70_sens, file = file.path(save_path, "dt_analysis_age70_sens2016.rds"))


dt_analysis_age50_sens %>% 
  group_by(algorithm) %>% 
  summarise(max_time = max(time_from_2010))

dt_analysis_age70_sens %>% 
  group_by(algorithm) %>% 
  summarise(max_time = max(time_from_2010))
